...
export const ampsProductsQuery = defineQuery(`*[_type == "product" && category == "amps" && archived != true] | order(featured desc, _createdAt desc) {
  _id,
  title,
  "slug": slug.current,
  "image": images[0]{
...
// Lesson Packages
export const allLessonPackagesQuery = defineQuery(`*[_type == "lessonPackage" && active == true && archived != true] | order(order asc, priceCents asc) {
  _id,
  title,
  "slug": slug.current,
  tagline,
...
// Events
export const upcomingEventsQuery = defineQuery(`*[_type == "event" && startDateTime >= $now && !isCanceled && archived != true] | order(startDateTime asc)[0...$limit]{
  _id,
  title,
  "slug": slug.current,
  excerpt,
...
export const pastEventsQuery = defineQuery(`*[_type == "event" && startDateTime < $now && archived != true] | order(startDateTime desc)[$offset...$limit]{
  _id,
  title,
  startDateTime,
  timezone,
...
export const eventsByMonthQuery = defineQuery(`*[_type == "event" && dateTime(startDateTime) >= dateTime($startOfMonth) && dateTime(startDateTime) < dateTime($endOfMonth) && archived != true] | order(startDateTime asc){
  _id,
  title,
  startDateTime,
  timezone,
...
export const eventsSlugs = defineQuery(`*[_type == "event" && archived != true]{
...
export const allProductsQuery = defineQuery(`*[_type == "product" && archived != true] | order(featured desc, _createdAt desc){
  _id,
  title,
  "slug": slug.current,
  "image": images[0]{
...
export const productSlugsQuery = defineQuery(`*[_type == "product" && defined(slug.current) && archived != true]{
  "slug": slug.current
}`)

export const relatedProductsByCategoryQuery = defineQuery(`*[_type == "product" && category == $category && _id != $excludeId && archived != true] | order(_createdAt desc)[0...$limit]{
  _id,
  title,
  "slug": slug.current,
  "image": images[0]{
...
export const featuredProductsQuery = defineQuery(`*[_type == "product" && featured == true && archived != true] | order(_createdAt desc)[0...$limit]{
  _id,
  title,
  "slug": slug.current,
  "image": images[0]{
...
// Product search
export const productSearchQuery = defineQuery(`*[_type == "product" && archived != true && (
...
// Sitemap
export const sitemapQuery = defineQuery(`*[_type in ["product", "event"] && defined(slug.current) && archived != true] | order(_type asc){
  "slug": slug.current,
  _type,
  _updatedAt,
  _type == "event" => {
